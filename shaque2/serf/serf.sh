...
SERFY_CONFIG_DIR="/etc/serf"
SERFY_LOG_FILE="/var/log/serf.log"

[[ -f /etc/sysconfig/serf ]] && . /etc/sysconfig/serf

[[ -d $SERFY_CONFIG_DIR ]] || mkdir -p $SERFY_CONFIG_DIR

start() {
    echo -n $"Starting $prog: "
    daemon --pidfile=$pidfile "${cmd} -config-dir=$SERFY_CONFIG_DIR >> $SERFY_LOG_FILE &"
    retval=$?
